...
try{
    ...
    return(
        <>
        <Card title="exp form" className="pb-5">
            ...
            <div className="w-full my-2">
                ...
                <select name='employeType'
                onChange={inputHandler}
                className="w-full border my-input-text">
                    {optEmployeType && optEmployeType.map((item,key)=>
...
<div className='w-full my-2'>
    ...
    onChange={inputHandler}
    className='w-full border my-input-text'>
        {optLocation &&optLocation.map((item,key)=>
        <option key={key} value={item.value}>{item.label}</option>
    )
        }
    </select>
